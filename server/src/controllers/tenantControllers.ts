...
export const getCurrentResidencies = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    res.json(residenciesWithFormattedLocation);
  } catch (error: any) {
    res.status(500).json({
      message: "Error retrieving tenant residencies",
      error: error.message,
    });
  }
};
...
export const addFavoriteProperty = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: `Error adding favorite properties: ${error.message}` });
  }
};
